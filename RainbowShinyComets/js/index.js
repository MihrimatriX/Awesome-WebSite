...
function loop_update() {
  ...
  meteors.map(function (meteor) {
    return meteor.update();
  });
  holes.map(function (hole, i) {
    return hole.dead ? holes.splice(i, 1) : hole.update();
  });
}
...
function loop_update_tick() {
  ...
  if (explosions > 0) explosions -= 1 / opts.hitCoolDown;

  someAreDead = false;
}
function loop_update_spawn() {

  if (meteors.length < opts.meteorCount && Math.random() < opts.meteorSpawnProb) meteors.push(new Meteor());
}
...
function loop_render_transform() {
  ...
  ctx.translate(Math.random() * opts.hitIntensity * explosions, Math.random() * opts.hitIntensity * explosions);
}
function loop_render_hitLine() {

  ctx.fillStyle = 'rgba(20,20,20,alp)'.replace('alp', opts.repaintAlpha * 2);
...
Meteor.prototype.update_ashes = function () {
  ...
  if (this.ashes.length < opts.ashCount && Math.random() < opts.ashSpawnProb) this.ashes.push(new Ash(this));

  this.ashes.map(function (ash) {
    return ash.update();
  });
};
Meteor.prototype.update_checkDead = function () {
  ...
  if (this.y > opts.hitLine) this.update_checkDead_explode();
};
...
Hole.prototype.update = function () {
  ...
  if (this.life > this.size * .60) this.dead = true;
};
